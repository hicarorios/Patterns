...
class PostMapper extends AbstractDataMapper implements PostMapperInterface
{
    ...
    public function __construct(DatabaseAdapterInterface $adapter, CommentMapperInterface $commenMapper)
    {
    ...
    {
        $post->id = $this->adapter->insert($this->entityTable, array("title"   => $post->title
    ...
    {
        $comments = $this->commentMapper->findAll(array("post_id" => $row["id"]));
